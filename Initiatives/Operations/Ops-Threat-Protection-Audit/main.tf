locals {
  ...
  policy_initiative       = "Ops-TP-Audit"
...
data "azurerm_policy_set_definition" "Ops-Threat-Protection-Audit" {
  name = local.policy_initiative
  management_group_name = data.azurerm_management_group.mg.name
  depends_on = [module.create_initiative_Ops_Threat_Protection_Audit]
}
