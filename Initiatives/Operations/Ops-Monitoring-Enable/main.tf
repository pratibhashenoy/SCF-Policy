locals {
  ...
  policy_initiative       = "Ops-Monitoring-Enable"
...
data "azurerm_policy_set_definition" "Ops-Monitoring-Enable" {
  name = local.policy_initiative
  management_group_name = data.azurerm_management_group.mg.name
  depends_on = [module.create_initiative_Ops_Monitoring_Enable]
}
