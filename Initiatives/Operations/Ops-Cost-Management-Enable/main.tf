locals {
  ...
  policy_initiative       = "Ops-Cost-Management-Enable"
}
...
data "azurerm_policy_set_definition" "Ops-Cost-Management-Enable" {
  name = local.policy_initiative
  management_group_name = data.azurerm_management_group.mg.name
  depends_on = [module.create_initiative_Ops_Cost_Management_Enable]
}
